...
int initialize_variables(int argc, char* argv[], struct Common_Variables * common_variables){
  ...
  for(int i=1;i<argc;i++){
    ...
    */	
	...
	if(str_arg.substr(0,7) != "--seed=" && str_arg.substr(0,6) != "--phi=" && str_arg.substr(0,7) != "--kmin=" && str_arg.substr(0,7) != "--kmax=" && str_arg.substr(0,5) != "--mu=" && str_arg.substr(0,9) != "--no-rate" && str_arg.substr(0,21) != "--uniform-amino-acids" && str_arg.substr(0,2) != "-m" && str_arg.substr(0,3) != "-o=" && str_arg.substr(0,8) != "--ofile=" && str_arg.substr(0,3) != "-T=" && str_arg.substr(0,8) != "--tRNAs=" && str_arg.substr(0,3) != "-A=" &&  str_arg.substr(0,13) != "--starting-A=" && str_arg.substr(0,13) != "--starting-T="&& str_arg.substr(0,8) != "--aaRSs=" && str_arg.substr(0,3) != "-N=" && str_arg.substr(0,10) != "--popsize=" && str_arg.substr(0,3) != "-n=" && str_arg.substr(0,2) != "-1" && str_arg.substr(0,2) != "-0" && str_arg.substr(0,11) != "--num-traj=" && str_arg.substr(0,5) != "--bp=" && str_arg.substr(0,5) != "--Mu=" && str_arg.substr(0,13) != "--Site-Types=" && str_arg.substr(0,18) != "--halting-fitness="&& str_arg.substr(0,19) != "--halting-fixation="&& str_arg.substr(0,3) != "-S="&&str_arg.substr(0,15) != "--codon-space-4"&& str_arg.substr(0,15) != "--codon-space-2" && str_arg.substr(0,13) != "--trans-bias=" && str_arg.substr(0,3) != "-i=" && str_arg.substr(0,8) != "--ifile=" && str_arg.substr(0,14) != "--proofreading" && str_arg.substr(0,3) != "-f=" && str_arg.substr(0,19) != "--no-double-mutants" && str_arg.substr(0,9) != "--no-mask"){
  ...
  std::cout<<"S (number of site-type) is "<<common_variables->N_site_type<<std::endl;
  ...
  log_file<<"Translation machinery substitution rate: mu = "<<common_variables->mu_id_feat<<std::endl;
  std::cout<<"mu (translation machinery substitution rate) is "<<common_variables->mu_id_feat<<std::endl;
  ...
  log_file<<"Transition bias: kappa = "<<common_variables->transition_bias<<std::endl;
  std::cout<<"kappa (transition bias) "<<common_variables->transition_bias<<std::endl;
  ...
  std::cout<<"fthalt (the halting fitness) is "<<common_variables->halting_fitness<<std::endl;
  log_file<<"The halting fixation: "<<common_variables->halting_fixation<<std::endl;
  std::cout<<"fxhalt (the halting fixation) is "<<common_variables->halting_fixation<<std::endl;
  ...
  if(bl_seed){
    std::cout<<"PRNG seed: "<<common_variables->seed<<std::endl;
    log_file<<"PRNG seed: "<<common_variables->seed<<std::endl;
  }
  else{
    std::cout<<"PRNG seed: "<<random_number_from_random_device<<std::endl;
